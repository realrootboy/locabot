...
class DesviosAdministrativo(Database.Base):
    __tablename__ = 'desvios_administrativo'
    id = Column(Integer, primary_key=True, autoincrement=True)
    administrativo_id = Column(Integer, ForeignKey('administrativos.id'))
    administrativo = relationship(
        'Administrativo', foreign_keys=[administrativo_id])
    ...
    responsavel = relationship(
        'Administrativo', foreign_keys=[responsavel_id])
